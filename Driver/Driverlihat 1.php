...
<script>
  ...
  // Menghapus event listener untuk tombol "Tambah"
     document.getElementById('openAddPopup').removeEventListener('click', loadAddPopup);

  fetch('Driveredit.php')
    .then(response => response.text())
    .then(data => {
      document.getElementById('popupContainer').innerHTML = data;
      // Mengisi nilai No. Driver pada pop-up edit
      document.getElementById('driverNo').value = driverNo;
